...
import { Card, CardBody, Chip, Button, Modal, ModalContent, ModalHeader, ModalBody, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell } from "@nextui-org/react";
...
function MisHijos() {
  ...
  const getNextRecommendedVaccine = (birthDate) => {
    const ageInMonths = calculateAgeInMonths(birthDate);
    const upcomingVaccines = esquemaVacunacion.filter(vaccine => {
      const minAge = vaccine.edad_minima_meses || 0;
  ...
  const getSelectedChild = () => {
    return hijos.find(hijo => (hijo.id_paciente || hijo.id_niño) === selectedChildId);
  };
    ...
    <div className="max-w-4xl mx-auto py-8">
      ...
      {loading && !error ? (
        ...
        <div className="space-y-4">
          {hijos.map((hijo) => {
            const recommendedVaccines = getRecommendedVaccines(hijo.fecha_nacimiento);
            const nextVaccine = getNextRecommendedVaccine(hijo.fecha_nacimiento);
                  ...
                  <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        ...
                        <p><span className="font-semibold">Edad:</span> {ageInMonths} meses</p>
                    ...
                    <div className="w-full md:w-auto md:min-w-[350px]">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <Button 
                          ...
                          color="primary" 
                          variant="flat"
                          onClick={() => showVaccinationSchedule(hijo.id_paciente || hijo.id_niño)}
                        >
                      ...
                      {recommendedVaccines.length > 0 && (
                        <div className="mb-3">
                          <p className="text-sm font-medium text-green-600 mb-2">Vacunas recomendadas para su edad:</p>
                          <div className="flex flex-wrap gap-1">
                            {recommendedVaccines.slice(0, 3).map((vaccine, index) => (
                              <Chip 
                                key={index} 
                                size="sm" 
                                color="success" 
                      ...
                      {nextVaccine && (
                        <div className="mb-3">
                          <p className="text-sm font-medium text-blue-600 mb-2">Próxima vacuna:</p>
                          <Chip size="sm" color="primary" variant="flat" className="text-xs">
                            {nextVaccine.nombre_vacuna} - {nextVaccine.edad_minima_meses} meses
                          </Chip>
                        </div>
                      )}
          ...
          <ModalHeader className="flex flex-col gap-1">
            ...
            {getSelectedChild() && (
              <p className="text-medium text-default-500">
                {getSelectedChild().nombre_completo} - {calculateAgeInMonths(getSelectedChild().fecha_nacimiento)} meses
              </p>
            )}
          </ModalHeader>
          <ModalBody>
              ...
              <div className="space-y-4">
                <Table aria-label="Calendario de vacunación">
                  ...
                  <TableBody>
                    {esquemaVacunacion
                      .sort((a, b) => (a.edad_minima_meses || 0) - (b.edad_minima_meses || 0))
                      .map((vaccine, index) => {
                        const ageInMonths = calculateAgeInMonths(getSelectedChild().fecha_nacimiento);
                        const minAge = vaccine.edad_minima_meses || 0;
                          ...
                          <TableRow key={index}>
                            <TableCell>
                              <div>
                                ...
                                {vaccine.descripcion && (
                                  <p className="text-xs text-default-400">{vaccine.descripcion}</p>
                                )}
                              </div>
                            </TableCell>
                            <TableCell>{minAge} meses</TableCell>
                            <TableCell>{maxAge === 1200 ? "Sin límite" : `${maxAge} meses`}</TableCell>
                ...
                <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  ...
                  <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
                    ...
                    <li>• Las fechas son aproximadas, el médico determinará el momento exacto</li>
